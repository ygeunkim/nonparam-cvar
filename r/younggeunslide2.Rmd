...
- Log returns $Y_t = \ln R_t$ corresponds *approximately to percentage changes*
- Use *negative log returns*
- VaR by **Upper quantile of the distribution of log return**

## VaR using log Return

$$\text{percentage change} \approx \ln \frac{P_{t + 1}}{P_t} = Y_{t + 1}$$

From $t$ to $t + 1$,

$$VaR = P_t \times VaR(Y_{t + 1})$$
...
\begin{cases}
  X_t = Y_{t - 1} \\
  Y_t = `r ar_mu` + `r ar` X_t + \sigma_t \epsilon_t \\
...
```{r, echo=FALSE}
garch_sim <- function(n, cond, ar_mu = ar_mu, ar = ar) {
  garch_spec <- 
    garchSpec(
      cond.dist = "norm",
      model = list(
        omega = omega, alpha = alp, beta = bet
      )
    )
...
$$Y_t \mid X_t \sim N \left( `r ar_mu` + `r ar` X_t, \sigma_t^2 \right)$$
