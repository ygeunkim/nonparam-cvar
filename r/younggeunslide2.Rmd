...
```{r, echo=FALSE, fig.cap="Bitcoin price in USD"}
coin %>% 
  ggplot(aes(Date, price)) +
  geom_line() +
  scale_x_date() +
...
#---------------------------------------------------
smile +
  annotate("segment", x = -.5, xend = -1.5, y = .5, yend = .6, arrow = arrow(length = unit(.03, "npc")), col = gg_hcl(1)) +
  annotate("text", x = -1, y = .6 - .01, label = "price more likely to fall if loss", col = "red") +
