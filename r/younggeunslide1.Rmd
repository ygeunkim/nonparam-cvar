...
- Quite general structure, reasonably weak condition
- $\mathcal{L}_a^b$: $\sigma$-algebra generated by $\{ (X_t, Y_t) \}_a^b$
- Strong mixing coefficient

$$\alpha(t) \defn \sup \left\{ \lvert P(AB) - P(A) P(B) \rvert \mathpunct{:} A \in \mathcal{L}_{-\infty}^0, B \in \mathcal{L}_t^{\infty} \right\}$$

- $\{ (X_t, Y_t) \}$ is a stationary $\alpha$-mixing if

$$\alpha(t) \rightarrow 0 \quad \text{as} \quad t \rightarrow \infty$$

## $\alpha$-Mixing Assumption

@cai:2008aa *assumed* that

$$\exists \delta_1 > 0 \mathpunct{:} \; \alpha(t) = O \left( t^{- (2 + \delta_1)} \right)$$

## Conditional CDF Bias

Under regularity conditions,

$$\sqrt{nh\lambda} \left[ \hat{f}(y \mid x) - f(y \mid x) - B_f(y \mid x) \right] \stackrel{d}{\rightarrow} N \left(0, \sigma_f^2(y \mid x) \right)$$

- $B_f(y \mid x) = \frac{\lambda^2}{2} \mu_2(W) f^{(2)}(y \mid x) + \frac{h^2}{2} \mu_2(K) f^{(2)} (y \mid x)$
- $\sigma_f^2(y \mid x) = \mu_0(K^2)\mu_0(W^2)\frac{f(y \mid x)}{g(x)}$

## Conditional CDF Bias

Under regularity conditions,

$$\sqrt{n\lambda} \left[ \hat{S}_{cai} (y \mid x) - S(y \mid x) - B_S(y \mid x) \right] \stackrel{d}{\rightarrow} N \left(0, \sigma_S^2(y \mid x) \right)$$

- $B_S(y \mid x) = \frac{\lambda^2}{2} \mu_2(W) S^{(2)}(y \mid x) + \frac{h^2}{2} \mu_2(K) f (y \mid x)$
    - If $h = o(\lambda)$, then $B_S(y \mid x) = \frac{\lambda^2}{2} \mu_2(W) S^{(2)}(y \mid x)$
- $\sigma_S^2(y \mid x) = \mu_0(W^2)S(y \mid x)\frac{1 - S(y \mid x)}{g(x)}$
